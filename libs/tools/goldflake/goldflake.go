...
const DefaultMachineID = 1

// DefaultStartTime Default Start Time
var DefaultStartTime = time.Date(2022, 7, 1, 0, 0, 0, 0, time.UTC)
...
func NewGoldflake(st Settings) error {
	gf = new(Goldflake)
	gf.mutex = new(sync.Mutex)
	gf.sequence = uint16(1<<BitLenSequence - 1)

	if st.StartTime.After(time.Now()) {
		return errors.New("data time err")
	}
...
func (gf *Goldflake) NextID() (string, error) {
	...
	tmp, err := gf.toID()
	return strconv.FormatUint(tmp, 16), err
}

const goldflakeTimeUnit = 1e6 // nsec, i.e. 1 msec
